...
**Remarque :** Certains développeurs en Swift expérimentés liront ce code, verront `try!`, puis m'enverront un courrier électronique incendiaire. Si vous envisagez de faire cela, lisez avant ce qui suit.
...
- La ligne `let path = Bundle.main.resourcePath!` déclare une constante appelée `path` à qui on assigne le chemin du paquet de notre application. Je vous rappelle qu'un paquet est un répertoire contenant notre programme compilé et toutes nos ressources. Ainsi, cette ligne indique : "dis-moi où je peux trouver toutes les images que j'ai ajoutées à mon application".
- La ligne `let items = try! fm.contentsOfDirectory(atPath: path)` déclare une troisième constante appelée `items` et lui assigne le contenu du répertoire du chemin. Quel chemin? Eh bien, celui qui a été retourné par la ligne précédante. Comme vous pouvez le constater, les noms des méthodes d'Apple sont relativement longs, rendant le code plutôt descriptif ! La constante `items` est une Array - une collection - contenant les noms de tous les fichiers trouvés dans le répertoire contenant les ressources de notre application.
- La ligne `for item in items {` commence une *boucle*. Les boucles sont un bloc de code qui s'exécute plusieurs fois. Dans le cas présent, la boucle s'exécute une fois pour chaque élément trouvé dans le paquet de l'application. Notez que la ligne se termine par une accolade ouvrante, indiquant le début d'un nouveau bloc de code, et de l'accolade fermante correspondante, quatre lignes plus bas. Tout ce qui se trouve à l'intérieur de ces accolades sera exécuté chaque fois itération de la boucle. Nous pourrions traduire cette ligne par "traite `items` comme une série de chaînes de caractères, puis extrais chacune de ces chaînes en lui donnant le nom `item`, puis exécute le code suivant…"
- La ligne `if item.hasPrefix("nssl") {` est la première ligne de notre boucle. À ce stade, nous sommes prêts à travailler avec le nom du premier fichier qui a été assigné à `item`. Pour décider s'il s'agit d'un nom de fichier qui nous intéresse, nous utilisons la méthode `hasPrefix()` : elle prend un paramètre (le préfixe à rechercher) et retourne true ou false. Le "if" du début signifie que cette ligne est une instruction conditionnelle : si l'élément a le préfixe "nssl", alors ... c'est vrai, une autre accolade ouvrante pour marquer un nouvel autre bloc de code. Cette fois, le code ne sera exécuté que si `hasPrefix()` a retourné la valeur true.
- Enfin, la ligne `// this is a picture to load!` (ceci est une image à charger!) est un commentaire - si nous arrivons jusqu'ici, `item` contient le nom d'une image à charger depuis notre paquet, nous devons donc la stocker quelque part.
