...
Cette fonction n'accepte aucun paramètre et retourne une chaîne de caractères : "Hate". Mais que se passe-t-il si la journée est particulièrement ensoleillée et que les haîneux (haters) ne se sentent pas détestés (hate) ? Eh bien, peut-être que nous ne voulons rien retourner : ce haîneux ne déteste personne aujourd'hui.
...
Pour l’instant, imaginez un sondage dans lequel vous demandez à une personne : "Sur une échelle de 1 à 5, comment évaluez-vous le talent de Taylor Swift ?". Que répondrait quelqu'un s'il n'avait jamais entendu parler d'elle ? 1 serait injuste envers elle et 5 l'encenserait alors qu'ils n'ont aucune idée de qui est Taylor Swift. La solution est les optionnels : "Je ne veux pas donner de numéro du tout."

Lorsque nous utilisons `-> String`, cela signifie "ceci retournera forcément une chaîne de caractères", ce qui signifie que cette fonction *ne peut pas* ne pas renvoyer de valeur, et peut donc être qualifiée de sûre, sachant qu'elle retournera toujours une valeur que nous pouvons utiliser comme une chaîne de caractères. Si nous voulions dire à Swift que cette fonction pourrait retourner une valeur ou non, nous devons utiliser ceci à la place :

    func getHaterStatus() -> String? {
        return "Hate"
    }

Remarquez le point d'interrogation supplémentaire : `String?` signifie "chaîne de caractères optionnelle". Maintenant, dans notre cas, nous renvoyons systématiquement "Hate", mais allons de l'avant et améliorons encore cette fonction : si le temps est ensoleillé (the weather is sunny), les haîneux (haters) ont tourné la page et ont abandonné leur vie de haine. Nous voulons donc ne rien retourner. En Swift, cette "absence de valeur" a un nom spécial : `nil`.
...
Cette fonction accepte un paramètre de type chaîne de caractères (weather) et retourne une chaîne de caractères (l'état du haîneux), mais cette valeur de retour peut être présente ou ne pas l'être (nil). Dans ce cas, cela signifie que nous pourrions obtenir une chaîne de caractères, ou nil.

Passons maintenant aux choses sérieuses : Swift veut que votre code soit vraiment sûr, et essayer d’utiliser une valeur nil est une mauvaise idée. Cela risquerait de faire crasher votre code, d'altérer la logique de faire afficher des choses incorrectes dans votre interface utilisateur. En conséquence, lorsque vous déclarez une valeur comme étant optionnelle, Swift veillera à la gérer en toute sécurité.
...
Pour résoudre le problème, nous devons modifier le type de la variable `status` en `String?` ou simplement supprimer entièrement l'annotation du type de données et laisser Swift déduire le type de données. La première option ressemble à ceci :
...
Swift a deux solutions. Les deux sont utilisées, mais l'une est certainement préférable à l'autre. La première solution s'appelle le déballage optionnel (optionnel unwrapping) et s'effectue dans une instruction conditionnelle à l'aide d'une syntaxe spéciale. Elle fait deux choses en même temps : vérifie si un optionnel a une valeur et, si tel est le cas, la déballe dans un type non optionnel, puis exécute un bloc de code.
...
OK, si vous êtes toujours là, ça signifie que vous n'avez pas du tout compris l'explication ci-dessus, ou que vous n'avez pas tout compris et que vous avez besoin de quelques précisions. Les optionnels sont très utilisés en Swift, vous devez donc absolument les comprendre. Je vais essayer d'expliquer à nouveau, d'une manière différente, et j'espère que cela aidera !
...
Cela donne 0, 1, 2, 0 - les positions de James et Bob sont les mêmes, même si l’un existe et que l’autre n’existe pas. C'est parce que j'ai utilisé 0 pour signifier "non trouvé". La solution facile pourrait être de retourner -1 pour non trouvé, mais que ce soit 0 ou -1, vous avez toujours un problème car vous devez vous rappeler que ce nombre spécifique signifie "non trouvé".
...
**Soyez prudent s'il vous plaît. Cependant, si vous essayez ceci sur une variable qui n'a pas de valeur, votre code va planter.**
...
Remarquez le point d'exclamation : cela signifie "Je suis certain que `year` contient une valeur, alors déballez-la de force immédiatement."
...
Vous rencontrerez principalement des optionnels implicitement déballés lorsque vous travaillerez avec des éléments d'interface utilisateur dans UIKit sur iOS ou AppKit sur macOS. Ceux-ci doivent être déclarées à l’avance, mais vous ne pouvez pas les utiliser avant qu'ils aient été créées - et Apple aime créer les éléments d’interface utilisateur au dernier moment afin d’éviter tout travail inutile. Avoir à déballer continuellement des valeurs alors que vous savez pertinemment qu’elles contiennent de la donnée peut vite s'avérer particulièrement énervant, elles sont donc implicitement déballées.

Ne vous inquiétez pas si vous trouvez que les optionnels implicitement déballés sont un peu difficiles à comprendre - cela deviendra clair au fur et à mesure que vous travaillerez avec Swift.
