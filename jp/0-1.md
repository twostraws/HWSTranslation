...
XcodeはAppleの開発者用プログラミングアプリケーションです。 これは、Mac App Storeから無料で入手でき、Appleのプラットフォーム用に開発を行うために必要です。 だから、最初のアクションは[Mac App StoreからXcodeをインストールするにはここをクリック](https://itunes.apple.com/us/app/xcode/id497799835?mt=12&at=10l8cn&ct=hws) です。これはかなりの 大きなダウンロードのため、まずダウンロードを開始し、読書を続けて下さい。ダウンロードされている間、絶対的な基本をいくつか説明するしましょう：
