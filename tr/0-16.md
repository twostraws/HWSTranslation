...
Bunun sebebi, hatırlarsanız daha önce söylediğimiz gibi, mutlaka bir değere sahip olması gereken iki `String` tipinde değer tanımladığımız içindir. Struct için bir sorun yok, çünkü Swift bizi bu iki özelliğe değer vermemiz için bizi zorlayarak, üyelik başlatmayı otomatik olarak üretiyor. Ama class'larda durum böyle değil, o yüzden Swift değer verileceğinden emin olamıyor.
...
Bunu yapmak için, bizi ilgilendiren iki parametre alan ve class içerisinde `init()` adında bir metod oluşturun:
...
İlki, `init()` metodundan önce `func` yazmayacaksınız, çünkü bu özel bir metoddur. İkincisi ise, parametre adları ile atamak istediğiniz özellik adları aynı olduğu için `self.` kullanın. Kolayca kavramanız şöyle söyleyebiliriz: "Bu nesnenin `clothes` özelliği, değerini aktaracağı `clothes` parametresiyle aynı olması gerekli". İstediğiniz isimleri verebilirsiniz onlara, tamamen size kalmış.
...
Basit birşeyle başlayalım: name (isim) ve age (yaş) özellikleri olan bir `Singer` class'ı oluşturalım. Metodlar olarak da, özelliklere değer atayabilmek için basit bir başlatıcı olacak içinde, bir de birkaç sözcük yazdıracak olan `sing()` metodu:
...
Şimdi `taylor` nesnesini oluşturulma şeklini değiştirin:
...
- O yüzden, önce özelliğimizi ayarlamamız gerekiyor (`noiseLevel`), ardından da kullanılacağı superclass için diğer parametreleri devredin.
...
Şimdilik `@objc` konusuna çok takılmayın; sadece devam eden konular içinde onu açıklamaya çalışmayacağım, aynı zamanda Xcode da her zaman bunu sana gerektiğinde söyleyecek. Alternatif olarak, eğer bireysel metodlar için `@objc` kullanmak istemezseniz, otomatik olarak Objective-C için tüm metodlarını class'ınız uygun hale getirmeden önce `@objcMembers` koyabilirsiniz.


## Değerler ve referanslar

Bir struct'ı kopyaladığınızda, tüm değerleriyle birlikte her şey çoğaltılır. Bu, bir struct'ın bir kopyasını değiştirerek, diğer kopyaların değiştiremeyeceğiniz anlamına gelir; hepsi bireyseldir. Class'larla, bir nesnenin her bir kopyası aynı orjinal nesneyi işaret eder. Yani birini değiştirirsen, hepsini değiştirmiş olursun. Swift struct'ları "değer tipleri" olarak tanımlar, çünkü onlar sadece bir değeri işaret der. Ama class'lar "referans tipleri"ndendir, çünkü nesneler gerçek değerlerin paylaşımlı referanslarıdır.

Bu önemli bir farktır ve struct'lar ile class'lar arasında seçimin önemli olduğu anlamına gelir

-   Eğer, değer geçilmesi için paylaşımlı ve yerinde değiştirilmiş bir duruma sahip olmak istiyorsanız, class'ları arıyorsunuz demektir. Fonksiyonların içine onları aktarırsınız veya dizilerde saklarsınız, orada değiştirirsiniz ve programın kalan kısmına bu değişiklikleri yansıtırsınız.
